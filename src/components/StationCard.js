export function createStationCard(station, isMonitored, onToggleMonitor, threshold = 1.0, onThresholdChange = null) {
    ...
    if (isMonitored) {
        ...
        <div style="display: flex; align-items: center; justify-content: space-between; margin-top: 1rem;">
            ...
            <div style="text-align: center;">
                 <label style="font-size: 0.75rem; color: #64748b; display: block;">Alarm Level (m)</label>
                 <input type="number" class="threshold-input" 
                    value="${threshold}" min="0.5" max="5.0" step="0.1" 
                    style="width: 80px; padding: 0.4rem; text-align: center; border: 1px solid #cbd5e1; border-radius: 4px; font-weight: bold;">
            </div>
        </div>

        <button class="btn btn-sm action-btn" style="width: 100%; margin-top: 1rem; background-color: #ef4444; color: white;">
            Stop Monitoring
        </button>
        `;
